...
The service is available at `http://localhost:8000`. In non-production environments, interactive Swagger UI is available at `/docs`, ReDoc at `/redoc`, and the OpenAPI document at `/openapi.json`. The versioned liveness endpoint is `GET /api/v1/health`.
